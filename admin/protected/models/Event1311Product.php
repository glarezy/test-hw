...
class Event1311Product extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('display, pid, title, desp, standard, performance, htmlhead, type', 'required'),
			array('recommend, seq', 'numerical', 'integerOnly'=>true),
			array('pid, title, type', 'length', 'max'=>255),
			array('ctime, mtime', 'length', 'max'=>11),
			array('language', 'length', 'max'=>50),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, pid, title, desp, standard, performance, recommend, seq, display, htmlhead, ctime, mtime, type', 'safe', 'on'=>'search'),
		);
	}
