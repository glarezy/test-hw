...
class ThumbHandler {
    ...
    function _output() {
        ...
        if (function_exists($func_name)) {
            ...
            if( $img_type == 'png' )
            	$this->setImgDisplayQuality(9);
            else
            	$this->setImgDisplayQuality(100);
    ...
    function _isFull() {
        Return ( $this->mask_w + $this->mask_offset_x > $this->fill_w
                || $this->mask_h + $this->mask_offset_y > $this->fill_h) ? true : false;
    }
...
//        }
    ...
    /**
     * 按照指定尺寸缩放和裁剪照片
     * @param unknown_type $_size 正方形尺寸
     */
    function cutToSize($_size) {
        //尺寸过小直接放回
        if ($this->src_w < $_size || $this->src_h < $_size)
            return false;

        //处理缩放
        if ($this->src_w > $this->src_h) {
            $w = ($_size / $this->src_h) * $this->src_w;
            $h = $_size;
        } else {
            $w = $_size;
            $h = ($_size / $this->src_w) * $this->src_h;
        }
        $this->createImg($w, $h);

        //处理裁剪
        $this->setSrcImg($this->dst_img);
        $this->setDstImg($this->dst_img);
        if ($this->src_w > $this->src_h) {
            $this->setSrcCutPosition(($this->src_w - $this->src_h) / 2, 0);
        } else {
            $this->setSrcCutPosition(0, ($this->src_h - $this->src_w) / 2);
        }
        $this->setRectangleCut($_size, $_size);
        $this->setCutType(2);
        $this->createImg($_size, $_size);
    ...
    function resize($width, $height) {
        ...
        if ($this->src_w < $width && $this->src_h < $height) {
            $this->createImg($this->src_w, $this->src_h);
	        return true;
	    }

        //处理缩放
        if ($this->src_w / $this->src_h > $width / $height ) {
        	return $this->cutToWidth($width);
            //$w = ($height / $this->src_h) * $this->src_w;
            //$h = $height;
        } else {
        	return $this->cutToHeight($height);
            //$w = $width;
            //$h = ($width / $this->src_w) * $this->src_h;
        }
        $this->createImg($w, $h);
    }

    function cutToRectangle($width, $height) {
        if ($this->src_w < $width || $this->src_h < $height)
        ...
        $this->setDstImg($this->dst_img);
        if ($this->src_w / $this->src_h > $width / $height) {
            $this->setSrcCutPosition(($this->src_w - $width) / 2, 0);
        } else {
            $this->setSrcCutPosition(0, ($this->src_h - $height) / 2);
        }
        $this->setRectangleCut($width, $height);
        $this->setCutType(2);
    ...
    /**
     * 按照指定宽度缩放照片
     * @param unknown_type $_size 宽度尺寸
     */
    function cutToWidth($_width) {
        //尺寸过小直接放回
        if ($this->src_w < $_width) {
        	$this->createImg($this->src_w, $this->src_h);
	        return true;
		}
        //处理缩放
        $w = $_width;
        $h = ($_width / $this->src_w) * $this->src_h;
        $this->createImg($w, $h);

        return true;
    }

    function cutToHeight($_height) {
        //尺寸过小直接放回
        if ($this->src_h < $_height) {
        	$this->createImg($this->src_w, $this->src_h);
	        return true;
		}
        //处理缩放
        $h = $_height;
        $w = ($_height / $this->src_h) * $this->src_w;
        $this->createImg($w, $h);
