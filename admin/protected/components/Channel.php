<?php
	class Channel {
		...
		public static function modSub($l1,$l2,$info) {
			$info['cnname'] = Channel::trimLabel($info['cnname']);
			$info['enname'] = Channel::trimLabel($info['enname']);
			$info['runame'] = Channel::trimLabel($info['runame']);

			if( !Channel::keyValid($info['key']) )
				return 'validkey';
			$res = Channel::get('all');

			if( $info['enname'] == '' || $info['cnname'] == '' || $info['runame'] == ''
				|| !isset($res['zh_cn'][$l1]) || !isset($res['en'][$l1]) || !isset($res['ru'][$l1])
				|| !isset($res['zh_cn'][$l1][$l2]) || !isset($res['en'][$l1][$l2]) || !isset($res['ru'][$l1][$l2]) )
				return 'paraerror';

			if( $info['key'] != $l2 ) {
				if( isset($res['zh_cn'][$l1][$info['key']]) || isset($res['en'][$l1][$info['key']]) || isset($res['ru'][$l1][$info['key']]) )
					return 'keyexists';
