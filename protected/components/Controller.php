...
/*		if( ! isset($cookie['lang']) ) {
			...
			$language = $cookie['lang']->value;

		$reqlang = Yii::app()->request->getParam('lang', '');
		if( $reqlang != '' && in_array($reqlang, array('zh_cn', 'en')) )
			$language = $reqlang;

		if( ! in_array($language, array('zh_cn', 'en')) )
			$language = 'zh_cn';
*/
...
//			|| $reqlang != ''
	...
	public function loadCssJs() {
		Yii::app()->clientScript->registerCssFile(Yii::app()->request->baseUrl.'/css/common.css?v=140220.css');
